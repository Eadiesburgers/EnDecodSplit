...
'Version: v2.5 - Added original encoded source filename to .par file name in hint.
'--------------------------------
...
if wscript.arguments.count < 3 then
	...
	case "d"
		if wscript.arguments.count < 3 then
			...
			if fcnCheckFolder(arguments(1)) then 
				...
				wscript.echo "Cannot be located: [" & arguments(1) & "]."
			end if 
		end if
	case "e" 
...
Function fEncodeB64(byref BinFileIn,B64FileOut)
	...
	OFile.writeline Replace(elem.text, vbLf, "")
	OFile.close
...
Function fcnFileOutIt(byref Filename,DataOut,FileOutCount,LeftToDo,RndSize)
	...
	'if not right(ParFolderOut,1)="\" then ParFolderOut=ParFolderOut & "\"
...
Function fcnHelper
	...
	vbcrlf & vbTab & vbTab & vbTab & vbTab & vbTab & vbTab & "--> Example: " & WScript.ScriptName & " E " & chr(34) & "c:\bobs folder\mail files\Big File.pdf" & chr(34) & " 9000 10000" & vbcrlf & "<" & lineVar
End Function
